...
const PanelProfesionalHeader = () => {
  ...
  const descargarPDF = () => {
    ...
    if (!usuario?.id_usuario) {
      alert("No se pudo obtener el ID del profesional para el reporte");
      console.log('id_profesional:', usuario.id_usuario);
      return;
    }
    
     // Verifica el objeto usuar

    const url = `http://localhost:5000/api/reporte-contable/${usuario.id_usuario}`;
    ...
    fetch(url, {
      ...
      .then((blob) => {
        ...
        link.href = fileURL;
        link.setAttribute('download', 'reporte-contable.pdf');
    ...
    <div className="header-seccion">
      <div className="intro">
        <p style={{ fontWeight: '500' }}>
          Bienvenido de vuelta, {usuario?.nombre || 'profesional'}
        </p>
      ...
      <div className="pdfContable">
        <FaRegFilePdf
          className="pdfIcon"
          style={{ paddingTop: '75px', color: '#2563eb', fontSize: '100px', fontWeight: '600' }}
        />
        <p style={{ color: 'lightgray', fontWeight: '400' }}>
          Descarga tu resumen en PDF
        </p>
        <div className="pdfBoton">
          <button className="descargarPDF" onClick={descargarPDF}>
            Descargar
          </button>
        </div>
      </div>
